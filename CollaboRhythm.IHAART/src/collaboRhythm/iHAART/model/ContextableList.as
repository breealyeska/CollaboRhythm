package collaboRhythm.iHAART.model
{
	...
	import collaboRhythm.iHAART.model.events.MobileContextMenuEvent;
	 ...
	 */
	[Event(name="itemLongPress", type="collaboRhythm.iHAART.model.events.ContextableListEvent")]

	/**
	 * Dispatched when an item of the context menu was clicked.
	 */
	[Event(name="contextMenuItemClick", type="collaboRhythm.iHAART.model.events.ContextableListEvent")]
	...
	public class ContextableList extends List
	{
		...
		protected function onMouseMove(event:MouseEvent):void
		{
			...
			if (Math.abs(_mouseDownPoint.x - pt.x) >
					DRAG_THRESHOLD ||
					Math.abs(_mouseDownPoint.y - pt.y) >
					DRAG_THRESHOLD)
			{
				_timer.removeEventListener(TimerEvent.TIMER_COMPLETE, onTimerComplete);
				_timer.stop();
		...
		protected function onTimerComplete(event:TimerEvent):void
		{
			...
			dispatchEvent(new ContextableListEvent(ContextableListEvent.ITEM_LONG_PRESS,
					false,
					false,
					mouseX,
					mouseY,
		...
		protected function showContextMenu():void
		{
			...
			_contextList.addEventListener(MobileContextMenuEvent.MENU_ITEM_CLICKED,
					onMobileContextMenuClicked,
					false,
					0,
					true);
		...
		{
			var evt:ContextableListEvent = new ContextableListEvent(ContextableListEvent.CONTEX_MENU_ITEM_CLICK,
					false,
					false,
					mouseX,
					mouseY,
